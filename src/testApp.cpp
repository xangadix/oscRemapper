...
void testApp::joystickRemap( float m, string a ) {
    ...
    if ( a == "/oscjoy.0.button.4") {
        ...
        if ( button_target != "/composition/direction") {
            // Random while up and down, and up c++
...
void testApp::doFourClips() {
    ...
    */
    ...
    // select clip
    /*
    if ( joy_axis_x > marge_high && joy_axis_y < marge_low && addr != "/layer2/clip"+ofToString(clip_nr+1)+"/connect" ) addr = "/layer2/clip"+ofToString(clip_nr+1)+"/connect";
    if ( joy_axis_x > marge_high && joy_axis_y > marge_high && addr != "/layer1/clip"+ofToString(clip_nr+1)+"/connect" ) addr = "/layer1/clip"+ofToString(clip_nr+1)+"/connect";
    if ( joy_axis_x < marge_low && joy_axis_y > marge_high && addr != "/layer2/clip"+ofToString(clip_nr)+"/connect" ) addr = "/layer2/clip"+ofToString(clip_nr)+"/connect";
    if ( joy_axis_x < marge_low && joy_axis_y < marge_low && addr != "/layer1/clip"+ofToString(clip_nr)+"/connect" ) addr = "/layer1/clip"+ofToString(clip_nr)+"/connect";
    ofxOscMessage s;
    s.setAddress(addr);
    s.addFloatArg(1);
    sender.sendMessage(s);
